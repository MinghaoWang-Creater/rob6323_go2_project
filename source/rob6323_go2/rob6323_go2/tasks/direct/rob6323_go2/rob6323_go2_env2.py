...
class Rob6323Go2Env(DirectRLEnv):
    ...
    def __init__(self, cfg: Rob6323Go2EnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # ================ 第1部分：动作历史记录 ================
        # 动作速率惩罚所需变量
        # 形状: (num_envs, action_dim, history_length)
        ...
        self._episode_sums = {
            ...
            for key in [
                ...
                "track_ang_vel_z_exp",
                "rew_action_rate",      # 第1部分：动作速率惩罚
                "raibert_heuristic",    # 第4部分：Raibert启发式
                "orient",               # 第5部分：姿态奖励
                "lin_vel_z",            # 第5部分：垂直速度奖励
                "dof_vel",              # 第5部分：关节速度奖励
                "ang_vel_xy",           # 第5部分：角速度奖励
                "feet_clearance",       # 第6部分：足部离地间隙奖励
                "contact_force",        # 第6部分：接触力奖励
            ]
        }
    ...
    def _reward_feet_clearance(self):
        """计算足部离地间隙奖励"""
        foot_positions = self.foot_positions_w[:, :, 2]  # Z坐标
        foot_clearance = foot_positions - self.robot.data.root_pos_w[:, 2].unsqueeze(1)
        
        # 期望的足部高度：摆动期间较高，站立期间较低
        desired_clearance = torch.where(
            self.desired_contact_states < 0.5,  # 摆动期
            torch.tensor(0.1, device=self.device),  # 期望高度10cm
            torch.tensor(-0.05, device=self.device)  # 站立期稍微下压
        )
        
        clearance_error = torch.abs(foot_clearance - desired_clearance)
    ...
    def _reward_contact_force(self):
        ...
        force_norms = torch.norm(contact_forces, dim=2)
        
        # 期望的接触力：站立期间有力量，摆动期间无力量
        desired_force = torch.where(
            self.desired_contact_states > 0.5,  # 站立期
            torch.tensor(50.0, device=self.device),  # 期望力量50N
            torch.tensor(0.0, device=self.device)    # 摆动期无力量
        )
        
        force_error = torch.abs(force_norms - desired_force)
        reward = torch.sum(torch.square(force_error), dim=1)
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # 更新先前动作历史（滚动缓冲区并插入新动作）
        self.last_actions = torch.roll(self.last_actions, 1, 2)
        self.last_actions[:, :, 0] = self._actions[:]
        
        # ================ 第4部分：步态更新 ================
        self._step_contact_targets()  # 更新步态状态
        ...
        rew_contact_force = self._reward_contact_force()
        
        # ================ 组合所有奖励 ================
        rewards = {
            "track_lin_vel_xy_exp": lin_vel_error_mapped * self.cfg.lin_vel_reward_scale,  # 移除了step_dt
            "track_ang_vel_z_exp": yaw_rate_error_mapped * self.cfg.yaw_rate_reward_scale,  # 移除了step_dt
            ...
            "contact_force": rew_contact_force * self.cfg.tracking_contacts_shaped_force_reward_scale,
        }
